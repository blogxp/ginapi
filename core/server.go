...
func RunServer() {
	...
	Server run :%d
	PID :%d
	`, global.VP.GetInt("serverini.HttpPort"), global.VP.Get("Pid"))
...
func GetServer() global.Server {
	if global.SER == nil {
		...
		global.SER = initServer(fmt.Sprintf("%s:%d", global.VP.GetString("serverini.HttpAddress"), global.VP.GetInt("serverini.HttpPort")), r)
	}
	return global.SER
}
